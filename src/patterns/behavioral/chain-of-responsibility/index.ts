export type HttpRequest = { path: string; method: string; headers: Record<string, string>; body?: unknown; userId?: string };
export type HttpResponse = { status: number; body: unknown };

export abstract class Middleware {
  private next?: Middleware;
  linkWith(next: Middleware): Middleware { this.next = next; return next }
...
export class RequestLogger extends Middleware {
  handle(request: HttpRequest){ console.log(`${request.method} ${request.path}`); return super.handle(request) }
}
...
export class AuthorizationMiddleware extends Middleware {
  handle(request: HttpRequest): HttpResponse {
    if (request.path.startsWith('/admin') && request.headers['x-role'] !== 'admin') return { status: 403, body: 'Insufficient permissions' };
    return super.handle(request);
  }
}
...
export class ProfileController extends Middleware {
  handle(request: HttpRequest): HttpResponse {
    if (request.path === '/profile' && request.method === 'POST') return { status: 200, body: { updatedBy: request.userId, changes: request.body } };
    return super.handle(request);
  }
}
...
export function createHttpPipeline(): Middleware {
  ...
  first.linkWith(new RateLimitMiddleware(10))
...
export function run(){
  ...
  console.log(pipeline.handle({ path: '/profile', method: 'POST', headers: { authorization: 'Bearer user-42', 'content-type': 'application/json' }, body: { name: 'Ada' } }));
