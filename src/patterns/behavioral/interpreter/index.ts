export type Product = { status: string; price: number; country: string; [key: string]: string | number };
export interface Expression { interpret(product: Product): boolean }
export class EqualsExpression implements Expression {
  constructor(private readonly field: string, private readonly expected: string) {}
  interpret(product: Product){ return String(product[this.field]).toLowerCase() === this.expected.toLowerCase() }
}
export class LessThanExpression implements Expression {
  ...
  interpret(product: Product){ return Number(product[this.field]) < this.limit }
}
export class AndExpression implements Expression {
  constructor(private readonly expressions: Expression[]) {}
  interpret(product: Product){ return this.expressions.every(expression => expression.interpret(product)) }
}
...
export function run(){
  ...
  const products: Product[] = [{ status: 'active', price: 79, country: 'ES' }, { status: 'active', price: 120, country: 'ES' }];
