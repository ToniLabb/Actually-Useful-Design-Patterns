...
export class EventBus {
  ...
  subscribe<T extends DomainEvent['type']>(type: T, handler: EventHandler<Extract<DomainEvent, { type: T }>>): () => void {
  ...
  publish(event: DomainEvent): void {
    for (const handler of [...(this.handlers.get(event.type) ?? [])]) {
      try { handler(event) } catch (error) { console.error(`Subscriber for ${event.type} failed:`, error) }
    }
  }
}
...
export class InvoiceGenerator {
  onOrderPaid(event: Extract<DomainEvent, { type: 'order.paid' }>): void { console.log(`Invoice generated for ${event.orderId}`) }
}
export class InventoryUpdater {
  onOrderPaid(event: Extract<DomainEvent, { type: 'order.paid' }>): void { console.log(`Inventory committed for ${event.orderId}`) }
}
