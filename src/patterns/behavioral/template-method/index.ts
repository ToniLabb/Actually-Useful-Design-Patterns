...
export abstract class DataImporter {
  ...
  protected read(source: string){ console.log(`Reading ${source}`); return source }
  protected abstract parse(raw: string): ImportedRecord[];
  protected validate(records: ImportedRecord[]){ if (records.length === 0) throw new Error('Import contains no records') }
  protected transform(record: ImportedRecord){ return { ...record, importedAt: new Date(0).toISOString() } }
  protected persist(records: ImportedRecord[]){ console.log(`Persisted ${records.length} records`) }
}
...
export class JsonImporter extends DataImporter {
  protected parse(raw: string): ImportedRecord[] { return JSON.parse(raw) as ImportedRecord[] }
}
export function run(){
