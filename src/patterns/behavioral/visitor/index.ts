...
export class HeadingNode implements DocumentNode {
  ...
  accept<T>(visitor: DocumentVisitor<T>) {
...
export function run() {
  const document: DocumentNode[] = [
    new HeadingNode(1, 'Useful patterns'),
    new ParagraphNode('Patterns solve recurring design problems.'),
    new LinkNode('Read more', '/patterns')
  ];
  console.log(document.map((node) => node.accept(new HtmlVisitor())).join(''));
  console.log(
    'Words:',
    document.reduce((total, node) => total + node.accept(new WordCountVisitor()), 0)
  );
}
