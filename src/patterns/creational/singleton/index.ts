...
export class AppConfig {
  ...
  static load(env: Environment = process.env): AppConfig {
    if (!this.instance) {
      ...
      this.instance = new AppConfig(environment as AppConfig['environment'], Number(env.PORT ?? 3000), env.DATABASE_URL);
    }
    return this.instance;
  }
  static resetForTests(){ this.instance = undefined }
}
export function run(){
  const first = AppConfig.load({ NODE_ENV: 'production', PORT: '8080', DATABASE_URL: 'postgres://db/app' });
