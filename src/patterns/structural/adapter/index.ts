...
export class StripeSdk {
  createPaymentIntent(input: { amountInCents: number; currency: string; token: string }) {
    return { id: 'pi_42', paid: input.token.startsWith('tok_') };
  }
}

export class PayPalSdk {
  executePayment(total: string, currencyCode: string, payerId: string) {
    return { transactionId: 'pp_42', state: payerId ? 'approved' : 'denied' };
  }
}
...
export class StripeAdapter implements PaymentGateway {
  ...
  charge(payment: Payment): PaymentResult {
    const result = this.stripe.createPaymentIntent({
...
export class PayPalAdapter implements PaymentGateway {
  ...
  charge(payment: Payment): PaymentResult {
    const result = this.paypal.executePayment(
    ...
    return {
      ...
      status: result.state === 'approved' ? 'succeeded' : 'failed'
    };
  }
}
...
export function run() {
  console.log(
    new BillingService(new StripeAdapter(new StripeSdk())).collect({
      amount: 49.99,
      currency: 'EUR',
      source: 'tok_visa'
    })
  ...
  console.log(
    new BillingService(new PayPalAdapter(new PayPalSdk())).collect({
      amount: 49.99,
      currency: 'EUR',
      source: 'payer-42'
    })
