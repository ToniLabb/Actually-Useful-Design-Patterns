...
export class FolderResource extends Resource {
  ...
  grant(userId: string, role: Role): void {
    super.grant(userId, role);
    for (const child of this.children) child.grant(userId, role);
  }
  revoke(userId: string): void {
    super.revoke(userId);
    for (const child of this.children) child.revoke(userId);
  }
  list(indent = ''): string[] {
    return [
      `${indent}${this.name}/`,
      ...this.children.flatMap((child) => child.list(`${indent}  `))
    ];
  }
}

export function run() {
  const project = new FolderResource('project')
    .add(new FileResource('proposal.pdf'))
    .add(new FolderResource('assets').add(new FileResource('logo.png')));
  project.grant('user-42', 'editor');
  console.log(
    project.list().join('\n'),
    'logo role:',
    (project as FolderResource).getRole('user-42')
  );
}
