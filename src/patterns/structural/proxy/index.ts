...
export interface ApiService { request(input: ApiRequest): ApiResponse }

export class RealApiService implements ApiService {
  request(input: ApiRequest): ApiResponse { return { status: 200, body: `Data for ${input.path}` } }
}
...
export function run(){
  ...
  for (let attempt = 1; attempt <= 3; attempt++) console.log(`Attempt ${attempt}:`, api.request({ clientId: 'client-1', path: '/reports' }));
}
